...
namespace Thaliak.Network.Utilities
{
    public sealed class Blowfish
    {
        ...
        public byte[] DecryptECB(byte[] ct) => this.Crypt_ECB(ct, true);
        ...
        private static byte WordByte0(uint w) => (byte)((w >> 24) & 0xFF);

        private static byte WordByte1(uint w) => (byte)((w >> 16) & 0xFF);

        private static byte WordByte2(uint w) => (byte)((w >> 8) & 0xFF);

        private static byte WordByte3(uint w) => (byte)(w & 0xFF);
        ...
        private void SetupKey(byte[] cipherKey)
        {
            ...
            for (var i = 0; i < 18; i++)
            {
                ...
                for (var k = 0; k < 4; ++k)
                {
                    // here we introduced a bug-as-a-feature
                    // which makes `d` almost always equal to 0xFFFF_FFFF
                    // this is identical to the behaviour of FFXIV lobby encryption
                    // DO NOT FIX IT
...
#pragma warning restore CS0675
            ...
            {
                this.Encipher();
                this.bfP[i] = this.xlPar;
                this.bfP[i + 1] = this.xrPar;
            }

            for (var i = 0; i < 256; i += 2)
            {
                this.Encipher();
                this.bfS0[i] = this.xlPar;
                this.bfS0[i + 1] = this.xrPar;
            }

            for (var i = 0; i < 256; i += 2)
            {
                this.Encipher();
                this.bfS1[i] = this.xlPar;
                this.bfS1[i + 1] = this.xrPar;
            }

            for (var i = 0; i < 256; i += 2)
            {
                this.Encipher();
                this.bfS2[i] = this.xlPar;
                this.bfS2[i + 1] = this.xrPar;
            }

            for (var i = 0; i < 256; i += 2)
            {
                this.Encipher();
                this.bfS3[i] = this.xlPar;
                this.bfS3[i + 1] = this.xrPar;
            }
        }
        ...
        private void Encipher()
        {
            ...
            {
                this.xrPar = this.Round(this.xrPar, this.xlPar, i + 1);
        ...
        private void Decipher()
        {
            ...
            {
                this.xrPar = this.Round(this.xrPar, this.xlPar, i);
